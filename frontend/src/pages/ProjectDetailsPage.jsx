...
const ProjectDetailsPage = () => {
  ...
  const { id } = useParams();
  const { user, fetchProjects } = useAuth();
  ...
  const handleUpdateTodo = async (todoId, todoData) => {
    try {
      ...
      setProjectTodos(
        projectTodos.map((todo) => (todo._id === todoId ? updatedTodo : todo))
      );
    } catch (err) {
    ...
    <>
      <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        <div className="w-full flex justify-center items-center">
          <h1 className="text-xl font-bold text-center inline-block mb-5">
            Project Details
          </h1>
        </div>
        {project.posterImage && (
        ...
        <span
          className={`inline-block ${
            project.status === "pending" ? "bg-yellow-500" : "bg-green-500"
          }  text-white rounded-full px-3 py-1 text-sm font-semibold mb-4`}
        >
